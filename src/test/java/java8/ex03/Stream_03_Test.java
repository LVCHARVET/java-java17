...
public class Stream_03_Test {
	...
	public void test_map_collect_joining() throws Exception {
		...
		// par le caractère "|"
		String result = customers.stream().map(cust -> cust.getFirstname()).sorted().collect(Collectors.joining("|"));
	...
	public void test_flatMap_distinct() throws Exception {
		...
		List<Pizza> result = orders.stream().flatMap(order -> order.getPizzas().stream()).distinct()
	...
	public void test_grouping() throws Exception {
		...
		Map<Customer, List<Order>> result = orders.stream().collect(Collectors.groupingBy(ord -> ord.getCustomer()));
